...
- Mehnat intizomi avto-vazifasi (`category="mehnat"`): tizimda **Mehnat intizomi mas'uli** (`employees.is_mehnat_responsible=true`) mavjud bo'lsa, qolgan barcha xodimlarning (mas'ulning o'zidan tashqari) ish rejasiga `enrichPlan` tomonidan avtomatik qo'shiladi. Bu Ijro mas'ulidan mustaqil — alohida checkbox staff formasida (emerald rang). Maydonlar: `mehnat_work_hours` (oylik ish soati), `mehnat_late_minutes` (kechikkan daqiqa), `mehnat_late_days` (kech kelgan kunlar), `mehnat_result` (matnli izoh). KPI = max(0, 100 - (lateMin / (workHours*60))*100). Faqat admin/manager yoki Mehnat mas'uli yangilashi mumkin (oddiy xodim/Ijro mas'uli — 403). Frontend'da emerald (yashil) rangli qator. Mehnat mas'uli tizimdan olib tashlansa, qator javobdan filtrlanadi (DB'da saqlanadi).
