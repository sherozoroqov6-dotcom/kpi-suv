...
- Mehnat intizomi avto-vazifasi (`category="mehnat"`): tizimda Ijro mas'uli mavjud bo'lsa, qolgan barcha xodimlarning (Ijro mas'ulining o'zidan tashqari) ish rejasiga `enrichPlan` tomonidan avtomatik qo'shiladi. Maydonlar: `mehnat_work_hours` (oylik ish soati), `mehnat_late_minutes` (kechikkan daqiqa), `mehnat_late_days` (kech kelgan kunlar), `mehnat_result` (matnli izoh). KPI = max(0, 100 - (lateMin / (workHours*60))*100). Faqat admin/manager yoki Ijro mas'uli yangilashi mumkin (oddiy xodim 403). Frontend'da emerald (yashil) rangli qator. Ijro mas'uli tizimdan olib tashlansa, qator javobdan filtrlanadi (DB'da saqlanadi).
